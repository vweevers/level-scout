  ...
  if (!paths.length) {
    ...
    if (plan.equiPaths.length > 1) {
      stream = plan.equiPaths.reduce(function(acc, path){
        return acc ? intersect(acc, keyStream(path), intersectKey) : keyStream(path)
      }, null)
...
function normalizeQuery(query) {
  ...
  Object.keys(query).forEach(function(property){
    var predicate = query[property]
    
    if (typeof predicate !== 'object') {
      normal[property] = { eq: predicate }
    } else if (('lt' in predicate) || ('lte' in predicate) || // todo: Object.keys()
               ('gt' in predicate) || ('gte' in predicate) ||
               ('eq' in predicate) ){
      normal[property] = predicate
    }
  })
