...
module.exports = function (db, query, opts) {
  ...
  if (!paths.length) {
    ...
    if (plan.equiPaths.length > 1) {
      ...
      // TODO: besides the number of matched predicates, sort the 
      // indexes by cardinality stats and maybe predicate selectivity (eq 
      // or a paired non-equi gets priority?).
