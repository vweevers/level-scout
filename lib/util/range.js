...
exports.matcher = function(expr) {
  if ('eq' in expr) return function(a, b) {
    return compare(a, encode(b)) === 0
  }.bind(null, encode(expr.eq))

  if ('gt' in expr)
    var gt = compare.bind(null, encode(expr.gt)), min = -1
  else if ('gte' in expr)
    gt = compare.bind(null, encode(expr.gte)), min = 0

  if ('lt' in expr)
    var lt = compare.bind(null, encode(expr.lt)), max = 1
  else if ('lte' in expr)
    lt = compare.bind(null, encode(expr.lte)), max = 0
