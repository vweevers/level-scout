...
Index.prototype.initializeStats = function() {
  ...
  this.length = 0 // Approximate number of total rows

  this.metadata.get('hll', function(err, prev){
    if (!prev) return

    this.length+= prev.length
    this.hll.merge(new Buffer(prev.state, 'base64'))
  }.bind(this))
}
...
Index.prototype.updateStats = function(id, add) {
  ...
  var hll = {
    state:  this.hll.state().toString('base64'),
    length: this.length
  }
...
Index.prototype.selectivity = function() {
  return this.length ? this.hll.count() / this.length : 0;
}
